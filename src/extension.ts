...
async function runInsertObjectNavigator(clickedUri?: vscode.Uri): Promise<void> {
    ...
    if (!fs.existsSync(appJsonPath)) {
        const proceed = await vscode.window.showWarningMessage(
            `No app.json found in "${workspaceFolder.name}". This doesn't look like an AL project root. Continue anyway?`,
            { modal: true },
            'Continue'
        );
        if (proceed !== 'Continue') {
    ...
    const appJson: AppJson = fs.existsSync(appJsonPath)
    ...
    if (objectId === undefined) {
        vscode.window.showErrorMessage(
            `Every page ID in this extension's ID range(s) (${idRanges
                .map((r) => `${r.from}..${r.to}`)
    ...
    vscode.window.showInformationMessage(
        `Created page ${objectId} "Page Navigator" at ${path.relative(workspaceFolder.uri.fsPath, filePath)}`
    );
}

/** Figure out which workspace folder to operate on (handles multi-root workspaces and right-click-in-explorer). */
async function resolveWorkspaceFolder(clickedUri?: vscode.Uri): Promise<vscode.WorkspaceFolder | undefined> {
    if (clickedUri) {
    ...
    const pick = await vscode.window.showQuickPick(
        ...
        { placeHolder: 'Select the AL project to add the Page Navigator page to' }
    );
    return pick?.folder;
}
...
function getIdRanges(appJson: AppJson): IdRange[] {
...
function scanUsedPageIds(rootPath: string): Set<number> {
...
function pickFreeId(ranges: IdRange[], used: Set<number>): number | undefined {
...
function buildPageSource(objectId: number): string {
    return `// ============================================================================
